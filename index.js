...
// Utility: send WhatsApp text (with optional idempotency key)
...
function resetTimer(phone, name) {
  ...
  // If thank-you already sent for this session, do nothing
  ...
  // Start a fresh 2-minute timer
  session.timer = setTimeout(async () => {
    // Extra guard
    if (session.hasThanked) return;

    session.hasThanked = true; // mark first to avoid races

    // Use last inbound message ID to make the send idempotent across instances
    ...
    // Clean up this session after sending
    if (session.timer) {
      clearTimeout(session.timer);
      session.timer = null;
    }
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // Update last inbound message id for idempotent thank-you
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    const from = msg.from;
    const text = interpretInput(msg.text?.body); // <-- normalize input here
    const name = contact?.profile?.name || "Customer";
