  ...
  try {
    ...
    if (statuses && statuses.length > 0) {
      for (const status of statuses) {
        ...
        if (statusType === "read") {
          console.log(`👁 User ${phone} opened template message: ${messageId}`);
          await logAction(phone, "Customer", "Template Opened", `MessageId: ${messageId}`);
        } else if (statusType === "delivered") {
          console.log(`📩 Template delivered to ${phone}`);
          await logAction(phone, "Customer", "Template Delivered", `MessageId: ${messageId}`);
        }
      }
      return res.sendStatus(200);
    }
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    const statuses = entry?.statuses;
    if (statuses && statuses.length > 0) return res.sendStatus(200);
...
const PHONE_ID = "749224044936223";
const TOKEN =
  "EAARCCltZBVSgBPJQYNQUkuVrUfVt0rjtNIaZBNVO7C24ZC5b5RO4DJKQOVZC5NWSeiknzZBrDec88QkAYYji7ypvDBgL1GDw3E39upO2TbuW8IfGx94VuH7bJpFKngdyJOjexp6SN6wYEM0Ah6MOERatzhjeth0sHeo8GneT6kyXyaPyHZA94Exe9NKVJZBIisrxAZDZD";
const GOOGLE_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbyKwi4iXPptEb3uJuOcybGf41_zYu69VqPmDYNh8qi1RyMJfv2isgxaZfHh788Cfka78g/exec";
...
async function logAction(phone, name, action, details = "") {
  try {
    if (!GOOGLE_SCRIPT_URL) return;
    await axios.post(GOOGLE_SCRIPT_URL, { userPhone: phone, customerName: name, action, details });
    console.log(`✅ Logged: ${action} - ${details}`);
  } catch (error) {
    console.error("❌ Logging failed:", error.response?.data || error.message);
  }
  ...
  try {
    ...
    const response = await axios.post(
      `https://graph.facebook.com/v23.0/${PHONE_ID}/messages`,
      {
        ...
        type: "template",
        template: {
          name: "intial_template_duplicate",
          language: { code: "en_US" },
...
app.get("/send", async (req, res) => {
  ...
  const result = await sendTemplate(phone, name || "Customer");
  if (result.success) res.send(`✅ Template sent to ${phone}`);
  else res.status(400).send(`❌ Failed: ${JSON.stringify(result.error)}`);
});
  ...
  try {
    ...
    if (statuses && statuses.length > 0) {
      for (const status of statuses) {
        ...
        if (statusType === "read") await logAction(phone, "Customer", "Template Opened", `MessageId: ${messageId}`);
        else if (statusType === "delivered")
          await logAction(phone, "Customer", "Template Delivered", `MessageId: ${messageId}`);
      }
      return res.sendStatus(200);
    ...
    if (buttonPayload) {
      if (buttonPayload.toLowerCase().includes("site"))
        ...
        await sendText(from, "📞 Please call us at +91-8008312211 for assistance.");
      await logAction(from, name, "Button Clicked", buttonPayload);
      return res.sendStatus(200);
    }
    ...
    if (userSession.stage === "main") {
      if (["1", "2", "3", "4"].includes(text)) {
        if (text === "1") {
          await sendText(from, `Available Projects:\n1️⃣ ${PROJECTS["1"].name}\n2️⃣ ${PROJECTS["2"].name}\n3️⃣ ${PROJECTS["3"].name}`);
          userSession.stage = "project_selection";
          await logAction(from, name, "Viewed Projects");
        } else if (text === "2") {
          await sendText(from, "📞 Call us: +91-8008312211\n📧 Email: abodegroups3@gmail.com\n🌐 Website: https://abodegroups.com");
          await logAction(from, name, "Talked to Expert");
        } else if (text === "3") {
          await sendText(from, `📄 Brochure Links:\n${Object.entries(PROJECTS)
            .map(([_, p]) => `${p.name}:\n2BHK\n${p.brochure["2BHK"]}\n3BHK\n${p.brochure["3BHK"]}`)
            .join("\n\n")}`);
          await logAction(from, name, "Downloaded Brochure");
        } else if (text === "4") {
          await sendText(from, "🗓 Book your site visit here: https://abodegroups.com/contact-us/");
          await logAction(from, name, "Booked Site Visit");
        }
      } else {
        await sendText(from, `✅ Hi ${name}, we received your query: "${rawText}". Our team will get back to you shortly!`);
        await logAction(from, name, "Custom Query", rawText);
    ...
    // Project selection and details handled similarly (as in your original code)
    // ... (keep your existing project_selection and project_details logic here)
     // Project selection stage
    else if (userSession.stage === "project_selection") {
      if (["1", "2", "3"].includes(text)) {
        const project = PROJECTS[text];
...
function resetTimer(phone, name) {
  ...
  session.timer = setTimeout(async () => {
    ...
    s.hasThanked = true;
    const idemKey = s.lastMessageId ? `ty-${phone}-${s.lastMessageId}` : `ty-${phone}-${Date.now()}`;
    await sendText(phone, `🙏 Thank you ${name} for connecting with Abode Constructions. Have a great day! ✨`, { idempotencyKey: idemKey });
    if (s.timer) clearTimeout(s.timer);
    delete sessions[phone];
  }, 2 * 60 * 1000);
}
